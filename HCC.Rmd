...
Per la comprobació de la distribució normal dels valors quantitatius utilitzaremos la prova de normalitat de Anderson-Darling.
...
var_normales<-tNorm$Variable[as.numeric(tNorm$p_value)>0.05]
...
## FALTA PONER LAS TABLAS CON KABLE.

```

Com es por veure, només dues de totes les variables quantitatives ( __Haemoglobin (g/dL)__ i __log_Gamma glutamyl transferase (U/L)__) segueixen una distribució normal i no tenen diferències significatives entre les variances entre la població que sobreviu i la que no. 
...
hcc_corTop <- hcc_corTop %>%
  select(variable, variable2, value) %>%
  rename(corr = value) %>% 
...
var_selecc<-varSig$Name[-c(7,10,13,17)]

hcc_sel<-hcc[c(var_selecc, "MELD", "Class Attribute")]

hcc_sel$`Ascites degree`<- as.numeric(hcc_sel$`Ascites degree`)
hcc_sel$`Encephalopathy degree`<- as.numeric(hcc_sel$`Encephalopathy degree`)
...
El model generat te només 7 variables i obté un 78% de precisió en les prediccions. 