...
for (i in hcc_num) {
  data <- hcc[,c(i,ind_CA )]
  name_var <- names(data)[1]
  a1<-data %>%
    ggplot(aes(x=data[,1], fill=`Class Attribute`))+
    geom_histogram() +
    labs(fill="Clase", y = "Frequencia", x =name_var ) +
    theme(legend.position = "bottom")
  
  a2<-data %>% 
    ggplot(aes(x=`Class Attribute`,y=data[,1])) +
    geom_boxplot() +
    labs(x = "Clase",  y = name_var)
...
for (i in hcc_log){
  name_var <- paste("log_", colnames(hcc)[i], sep = '')
  hcc[,i]<- log(hcc[,i])
  colnames(hcc)[i] <- name_var
  data <- hcc[,c(i,ind_CA )] 
...
for (i in hcc_factorT) {
  ...
  a1<-data %>% 
    ggplot(aes(x=data[,1],fill=`Class Attribute`))+ 
    geom_bar() +
    labs(fill="Clase", x = name_var, y = "Frequencia") + 
    theme(legend.position = "bottom")
  
  a2<- data %>% 
    ggplot(aes(x=data[,1],fill=`Class Attribute`))+ 
    geom_bar(position = "fill") +
    labs(fill="Clase", x = name_var, y = "Proporcio") +
    theme(legend.position = "bottom")
  
  grid.arrange(a1,a2,nrow=1,top = textGrob(name_var,gp=gpar(fontsize=15)))
}
```
...
#modificar ultima fila Na per  epsais en blancs

kable(x = tau, format = "latex", caption = "Variables que no segueixen una distribució normal", booktabs = TRUE, digits = 4) %>% 
  kable_styling(latex_options = c("HOLD_position", "scale_down"))
