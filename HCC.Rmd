...
# 
# hcc3$Symptoms<-kNN(hcc3)$Symptoms
# hcc3$Hemochromatosis<-kNN(hcc3)$Hemochromatosis
# hcc3$`Esophageal Varices`<-kNN(hcc3)$`Esophageal Varices`


#computem tots els valors NA de variables factor
cn<- colnames(hcc[hcc_factorT,])

hcc2 <- kNN(hcc, variable = cn) %>% subset(select = Gender:`Class Attribute`)
summary(hcc2)

#Falta canviar el tex t d'abans, ja que no penso que imputar la moda en variables binaries sigui el mateix. Mira el cas d'Smooking i ho veuras clar.
```
