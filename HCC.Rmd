...
hcc_colNA<-names(hcc[,colSums(is.na(hcc))>15])
hcc_colNA20<-c()
...
for (i in hcc_colNA)
  {
    print (i)
    aa<-table(hcc$`Class Attribute`[is.na(hcc[,i])])[1]/sum(is.na(hcc[,i]))-(59/156)
    if (abs(aa)>0.2){
...
cat("\nVariables con >20% de diferencia de NA con lo esperado\n",hcc_colNA20)
